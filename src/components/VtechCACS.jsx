...
const VtechCACS = () => {
  ...
  return (
    <>
      ...
      {/* CS ACHIEVERS */}
      <section id="achievers-2" className="text-white d-flex flex-column justify-content-center align-items-center px-3 py-5" style={{ minHeight: "100vh", scrollMarginTop: "60px" }}>
        <h2 className="display-4 fw-bold mb-1 text-center text-gradient" data-aos="fade-down" style={{ color: '#8c5278' }}>
          🌟 Sculpting Future Company Secretaries with Purpose
        </h2>
        <p className="text-base text-dark max-w-2xl mx-auto leading-relaxed mb-5 pb-5">
          From rank-holders to corporate-ready professionals — we shape every CS aspirant with mentorship, discipline, and direction.
        </p>
        <div className="container">
          <div className="row g-4">
            {csHighlights.map((card, idx) => (
              <div key={idx} className={`col-md-6 d-flex ${idx % 2 === 0 ? "flex-row" : "flex-row-reverse"}`} data-aos="fade-up">
                <div className="card card-custom w-100 text-white overflow-hidden">
                  <img src={card.img} alt={card.title} className="img-fluid w-100 h-100 object-fit-cover" style={{ height: "300px", objectFit: "cover" }} />
                  <div className="card-content bg-dark bg-opacity-50 p-4 position-absolute bottom-0 w-100">
                    <h3 className="card-title" style={{ color: '#e792c9ff' }}>{card.title}</h3>
                    <p className="card-text">{card.desc}</p>
                  </div>
                </div>
              </div>
